...
```bash
pip install -r requirements.txt   # pytest (optional)
```
...
- Demonstrates OS-level concurrency — threads, locks, race conditions — which is what networking interviews test
- Simpler to reason about in a blocking I/O context
- Scales fine for LAN chat — 50-100 clients is well within thread limits
...
Key invariant: only one thread ever reads from a given client socket (the `ClientHandler` for that client). Writes can come from multiple threads (e.g. a broadcast triggered by another client's message), so `_send_lock` serialises them.
...
- TLS encryption for message traffic
- Token/password-based authentication instead of plain usernames
- Persistent message history (e.g. SQLite)
- Horizontal scaling across multiple server instances via a shared message broker
